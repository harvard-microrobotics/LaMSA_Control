...
vL_array = vL_thresh.*beta; %m/s

for i = 1:length(vL_array)
    [t_s x_s v_s a_s F_s] = spring_driven_dynamics(deltaX*1e-3,L*1e-3,k,R*1e-3,vL_array(i),m*1e-3,m_s*1e-3);
...
%    vL_array_bar = round(vL_array./(v_recoil.*sqrt(R_bar)),2);
   vL_array_bar = vL_array./vL_thresh;
...
c2Label = xlabel(c2, '$\tilde{v}_{L}$','interpreter','Latex');

for i = 1:length(vL_array)
    plot_color = colorMap(round(vL_array_bar(i)*10-min_c+1),:);
    p1(i) = plot(timeArray{i},energyOutput{i},'Color',plot_color);
    if normalizecommand
        legch(i) = cellstr(['$\tilde{v}_L$ = ',num2str(vL_array_bar(i))]);
    else
        legch(i) = cellstr(['$v_L$ = ',num2str(vL_array(i)),'m/s']);
    end
...
% if (savecommand)
%     fpath = strcat(pwd,folderName);
%     filename1 = strrep(['ContactLatch_ForceTime_Velocity'],'.','p');
%     saveas(gcf,fullfile(fpath,filename1),'fig');
%     saveas(gcf,fullfile(fpath,filename1),'png');
%     saveas(gcf,fullfile(fpath,filename1),'svg');  
% end
